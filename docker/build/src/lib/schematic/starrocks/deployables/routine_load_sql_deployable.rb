    ...
    module Deployables
      class RoutineLoadSqlDeployable < SqlDeployable
        ALLOWED_SQL_PATTERNS = [
          /\ACREATE\s+ROUTINE\s+LOAD\s+(?<db_name>[\w.]+)\.(?<load_name>\w+)\s+ON\s+/i,  # Match CREATE ROUTINE LOAD db.name ON
          /\s+COLUMNS\s*\([^)]+\)/i,                                                     # contain COLUMNS definition
          /\s+PROPERTIES\s*\([^)]+\)/i                                                   # contain PROPERTIES definition
        ].freeze

        ALLOWED_COMMANDS = [
          /\AUSE\s+\w+/i,                          # Allow USE statement
          /\ACREATE\s+ROUTINE\s+LOAD/i,
          /\APAUSE\s+ROUTINE\s+LOAD\s+FOR/i,
          /\ARESUME\s+ROUTINE\s+LOAD\s+FOR/i,
          /\ASTOP\s+ROUTINE\s+LOAD\s+FOR/i,
          /\AALTER\s+ROUTINE\s+LOAD\s+FOR/i        # ALTER command
        ].freeze
